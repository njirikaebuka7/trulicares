...
export default function AssistantWidget() {
  ...
  const panelOffset = isDashboardLike ? 'bottom-24' : 'bottom-5';
  ...
  return (
    <div className={cn('fixed right-4 z-[950] sm:right-6', launcherOffset)}>
      {open && (
        <div
          className={cn(
            'fixed left-3 right-3 z-[960] overflow-hidden rounded-[28px] border border-brand-100 bg-white shadow-[0_24px_80px_rgba(18,59,46,0.2)] sm:left-auto sm:right-6 sm:w-[24rem]',
            panelOffset
          )}
        >
          <div className="relative overflow-hidden bg-gradient-to-br from-brand-900 via-brand-800 to-brand-950 px-5 pb-5 pt-4 text-white">
              ...
              <div className="min-w-0">
                <div className="mb-2 flex items-center gap-2">
                  <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-white/10 backdrop-blur">
                    <Bot className="h-5 w-5 text-brand-100" />
                  </div>
                ...
                <h2 className="text-lg font-bold">TruliCares Assistant</h2>
                <p className="mt-1 max-w-xs text-sm text-brand-100/85">
                  Care, staffing, and account guidance whenever you need it.
                </p>
              </div>
              <button
          ...
          <div className="bg-[linear-gradient(180deg,#fffaf4_0%,#ffffff_24%)]">
            <div ref={messagesRef} className="max-h-[24rem] min-h-[22rem] space-y-4 overflow-y-auto px-4 py-4">
              {loadingConversation ? (
                  ...
                  <div className="grid gap-2">
                    {starterPrompts.slice(0, 4).map((prompt) => (
                      <button
                        ...
                        className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-left text-sm font-medium text-slate-700 transition hover:border-brand-200 hover:bg-brand-50/60 hover:text-brand-800"
                      >
                        {prompt}
                      </button>
                    ))}
            ...
            <div className="border-t border-slate-100 bg-white px-4 pb-4 pt-3">
              <div className="mb-3 flex items-center gap-2 rounded-2xl bg-slate-50 px-3 py-2 text-xs text-slate-500">
              ...
              <div className="rounded-[24px] border border-slate-200 bg-white p-2 shadow-sm">
                ...
                <div className="mt-2 flex items-center justify-between gap-3">
                  ...
                  <button
                    ...
                    className="inline-flex items-center gap-2 rounded-full bg-brand-700 px-4 py-2 text-sm font-semibold text-white transition hover:bg-brand-800 disabled:cursor-not-allowed disabled:bg-slate-300"
                  >
                    Send
      ...
      <button
        ...
        className="group flex items-center gap-3 rounded-full border border-brand-200 bg-white px-4 py-3 shadow-[0_18px_42px_rgba(14,58,47,0.16)] transition hover:-translate-y-0.5 hover:shadow-[0_22px_48px_rgba(14,58,47,0.2)]"
        aria-label="Open TruliCares Assistant"
      >
        <span className="flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-br from-brand-700 to-brand-500 text-white shadow-inner">
