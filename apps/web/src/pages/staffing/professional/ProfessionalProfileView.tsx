      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="px-6 pb-6">
          <div className="relative flex flex-col md:flex-row items-end gap-6 -mt-12 mb-6">
            <div className="w-32 h-32 rounded-xl bg-white p-1.5 shadow-md">
            ...
            <div className="flex gap-3">
              {editing ? (
                <>
                  <button onClick={() => setEditing(false)} className="px-5 py-2 border border-gray-200 text-gray-600 font-semibold rounded-full hover:bg-gray-50 transition-all flex items-center gap-2 active:scale-95 text-sm">
                    <X className="w-4 h-4" /> Cancel
                  </button>
                  <button onClick={handleSave} className="px-5 py-2 bg-brand-600 text-white font-semibold rounded-full hover:bg-brand-700 transition-all flex items-center gap-2 shadow-sm active:scale-95 text-sm">
                    <Save className="w-4 h-4" /> Save Changes
                  </button>
                </>
              ) : (
                <button onClick={() => setEditing(true)} className="px-5 py-2 bg-gray-900 text-white font-semibold rounded-full hover:bg-brand-600 transition-all flex items-center gap-2 shadow-sm active:scale-95 text-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            {editing ? (
              <textarea 
                className="w-full p-4 bg-gray-50 border border-gray-100 rounded-xl text-sm font-medium resize-none outline-none focus:ring-2 focus:ring-brand-500 transition-all"
                rows={6}
                value={form.bio}
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            <div className="flex flex-wrap gap-2.5">
              {profile?.specialties.map((s, i) => (
                <div key={i} className="px-3.5 py-1.5 bg-brand-50 text-brand-700 rounded-xl text-xs font-bold border border-brand-100 shadow-sm">
                  {s}
                </div>
              ))}
              {editing && (
                <button className="px-3.5 py-1.5 border border-dashed border-gray-200 text-gray-400 rounded-xl text-xs font-semibold hover:border-brand-300 hover:text-brand-500 transition-all flex items-center gap-1.5">
                  <PlusCircle className="w-3.5 h-3.5" /> Add Specialty
                </button>
              )}
            </div>
          </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm">
            ...
            <div className="space-y-3">
              {[
                ...
                <div key={i} className="flex items-center justify-between p-3.5 bg-gray-50 rounded-xl border border-gray-100 group hover:border-brand-200 transition-colors">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-lg bg-white flex items-center justify-center text-gray-400 group-hover:text-brand-600 shadow-sm transition-colors">
                      <doc.icon className="w-4 h-4" />
                    </div>
                    <div>
                      <p className="text-sm font-bold text-gray-900">{doc.label}</p>
                      <p className={`text-[10px] font-semibold uppercase tracking-wider ${doc.status === 'Verified' ? 'text-emerald-500' : 'text-amber-500'}`}>{doc.status}</p>
                    </div>
                  </div>
          ...
          <div className="bg-gradient-to-br from-emerald-600 to-teal-700 rounded-2xl p-6 text-white shadow-sm">
            ...
            <div className="space-y-4">
              ...
              <p className="text-[10px] text-emerald-100 font-medium opacity-85">Add a professional photo and certifications to reach 100% and get priority shift matching.</p>
            </div>
          </div>
        </div>
      </div>
