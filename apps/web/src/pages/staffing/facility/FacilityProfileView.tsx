...
import { 
  Building2, Globe, Phone, FileText, MapPin, 
  Shield, CheckCircle, Clock, Save, Edit3, 
  X, Loader2, UserCircle, Briefcase, Mail, Info
} from 'lucide-react';
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-fade-in">
      ...
      <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden">
        <div className="h-36 bg-gradient-to-r from-brand-700 via-brand-800 to-gray-900" />
        <div className="px-8 pb-8">
          <div className="relative flex flex-col md:flex-row items-end gap-6 -mt-12 mb-6">
            ...
            <div className="flex-1 pb-2">
              <h2 className="text-3xl font-black text-gray-900 tracking-tight">{profile?.facility_name}</h2>
              ...
              {editing ? (
                <>
                  <button 
                    ...
                    className="px-6 py-3 border-2 border-gray-100 text-gray-600 font-bold rounded-2xl hover:bg-gray-50 transition-all flex items-center gap-2"
                  >
                    <X className="w-4 h-4" /> Cancel
                  </button>
                  <button 
                    onClick={handleSave} 
                    className="px-6 py-3 bg-brand-600 text-white font-bold rounded-2xl hover:bg-brand-700 transition-all flex items-center gap-2 shadow-lg shadow-brand-100"
                  >
                ...
                <button 
                  ...
                  className="px-6 py-3 bg-gray-900 text-white font-bold rounded-2xl hover:bg-brand-600 transition-all flex items-center gap-2 shadow-lg shadow-gray-200"
                >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-[2.5rem] p-8 lg:p-10 border border-gray-100 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-xs font-black uppercase tracking-wider text-gray-400 mb-2 ml-1">Facility Name</label>
                {editing ? (
                  <input
                    type="text"
                  ...
                  <p className="text-gray-800 text-sm font-bold p-3 bg-gray-50/50 rounded-2xl">{profile?.facility_name || 'N/A'}</p>
                )}
              </div>

              <div>
                <label className="block text-xs font-black uppercase tracking-wider text-gray-400 mb-2 ml-1">Facility Type</label>
                {editing ? (
                  <select
                    className="w-full px-4 py-3 bg-gray-50 border-2 border-transparent focus:border-brand-500 rounded-2xl text-sm font-medium outline-none transition-all appearance-none"
                  ...
                  <p className="text-gray-800 text-sm font-bold p-3 bg-gray-50/50 rounded-2xl">{currentTypeLabel}</p>
                )}
              </div>

              <div>
                <label className="block text-xs font-black uppercase tracking-wider text-gray-400 mb-2 ml-1">EIN (Employer ID Number)</label>
                {editing ? (
                  <input
                    type="text"
                    className="w-full px-4 py-3 bg-gray-50 border-2 border-transparent focus:border-brand-500 rounded-2xl text-sm font-medium outline-none transition-all"
                  ...
                  <p className="text-gray-800 text-sm font-bold p-3 bg-gray-50/50 rounded-2xl">{profile?.ein || 'N/A'}</p>
                )}
              </div>

              <div>
                <label className="block text-xs font-black uppercase tracking-wider text-gray-400 mb-2 ml-1">Business Phone</label>
                {editing ? (
                  <input
                    type="text"
                    className="w-full px-4 py-3 bg-gray-50 border-2 border-transparent focus:border-brand-500 rounded-2xl text-sm font-medium outline-none transition-all"
              ...
              <div className="md:col-span-2">
                <label className="block text-xs font-black uppercase tracking-wider text-gray-400 mb-2 ml-1">Street Address</label>
                {editing ? (
                  <input
                    type="text"
                    className="w-full px-4 py-3 bg-gray-50 border-2 border-transparent focus:border-brand-500 rounded-2xl text-sm font-medium outline-none transition-all"
                  ...
                  <p className="text-gray-800 text-sm font-bold p-3 bg-gray-50/50 rounded-2xl">{profile?.address || 'N/A'}</p>
                )}
              </div>

              <div>
                <label className="block text-xs font-black uppercase tracking-wider text-gray-400 mb-2 ml-1">City</label>
                {editing ? (
                  <input
                    type="text"
                    className="w-full px-4 py-3 bg-gray-50 border-2 border-transparent focus:border-brand-500 rounded-2xl text-sm font-medium outline-none transition-all"
                  ...
                  <p className="text-gray-800 text-sm font-bold p-3 bg-gray-50/50 rounded-2xl">{profile?.city || 'N/A'}</p>
                )}
              </div>
              ...
              <div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-black uppercase tracking-wider text-gray-400 mb-2 ml-1">State</label>
                    {editing ? (
                      <input
                        type="text"
                        className="w-full px-4 py-3 bg-gray-50 border-2 border-transparent focus:border-brand-500 rounded-2xl text-sm font-medium outline-none transition-all text-center uppercase"
                        value={form.state}
                        onChange={(e) => setForm({ ...form, state: e.target.value })}
                        maxLength={2}
                      />
                    ) : (
                      <p className="text-gray-800 text-sm font-bold p-3 bg-gray-50/50 rounded-2xl text-center">{profile?.state || 'N/A'}</p>
                    )}
                  </div>
                  <div>
                    <label className="block text-xs font-black uppercase tracking-wider text-gray-400 mb-2 ml-1">ZIP Code</label>
                    {editing ? (
                      <input
                        type="text"
                        className="w-full px-4 py-3 bg-gray-50 border-2 border-transparent focus:border-brand-500 rounded-2xl text-sm font-medium outline-none transition-all text-center"
                        value={form.zip}
                        onChange={(e) => setForm({ ...form, zip: e.target.value })}
                      />
                    ) : (
                      <p className="text-gray-800 text-sm font-bold p-3 bg-gray-50/50 rounded-2xl text-center">{profile?.zip || 'N/A'}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-black uppercase tracking-wider text-gray-400 mb-2 ml-1">Website URL</label>
                {editing ? (
                  <div className="relative">
                    <Globe className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                    <input
                      type="text"
                      className="w-full pl-12 pr-4 py-3 bg-gray-50 border-2 border-transparent focus:border-brand-500 rounded-2xl text-sm font-medium outline-none transition-all"
                      value={form.website}
                  ...
                  <div className="p-3 bg-gray-50/50 rounded-2xl flex items-center justify-between">
          ...
          <div className="bg-white rounded-[2.5rem] p-8 lg:p-10 border border-gray-100 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-xs font-black uppercase tracking-wider text-gray-400 mb-2 ml-1">Contact Name</label>
                {editing ? (
                  <input
                    type="text"
                    className="w-full px-4 py-3 bg-gray-50 border-2 border-transparent focus:border-brand-500 rounded-2xl text-sm font-medium outline-none transition-all"
                  ...
                  <p className="text-gray-800 text-sm font-bold p-3 bg-gray-50/50 rounded-2xl">{profile?.contact_name || 'N/A'}</p>
                )}
              </div>

              <div>
                <label className="block text-xs font-black uppercase tracking-wider text-gray-400 mb-2 ml-1">Contact Title</label>
                {editing ? (
                  <input
                    type="text"
                    className="w-full px-4 py-3 bg-gray-50 border-2 border-transparent focus:border-brand-500 rounded-2xl text-sm font-medium outline-none transition-all"
                  ...
                  <p className="text-gray-800 text-sm font-bold p-3 bg-gray-50/50 rounded-2xl">{profile?.contact_title || 'N/A'}</p>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="space-y-8">
          ...
          <div className="bg-gradient-to-br from-gray-900 via-brand-950 to-black rounded-[2.5rem] p-8 text-white shadow-xl relative overflow-hidden group">
            <div className="absolute top-0 right-0 w-32 h-32 bg-brand-500/10 rounded-full blur-[50px] -mr-10 -mt-10" />
            <h4 className="text-lg font-black tracking-tight mb-4 flex items-center gap-2">
              <Shield className="w-5 h-5 text-brand-400" /> Facility Profile Grade
            </h4>
            <div className="space-y-4 relative">
              <div>
                ...
                <div className="h-2 bg-gray-800 rounded-full overflow-hidden">
                  <div className="h-full bg-brand-500 w-full rounded-full shadow-[0_0_10px_rgba(16,185,129,0.5)]" />
                </div>
              </div>
