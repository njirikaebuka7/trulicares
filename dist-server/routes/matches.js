...
const router = Router();
function buildCaregiverProfile(row) {
...
function formatFamilyMatch(row) {
    ...
    const messagingUnlocked = (row.messaging_unlocked || false) && (!careDate || hoursElapsed <= 48);
    return {
        id: row.id,
...
function formatCaregiverMatch(row) {
    return {
        id: row.id,
...
router.get('/', requireAuth, async (req, res) => {
    try {
        ...
        if (role === 'family') {
            const result = await query(`SELECT m.id, m.request_id, m.caregiver_id, m.family_id, m.status,
         ...
         ORDER BY m.near_you DESC, cp.rating DESC NULLS LAST, m.created_at DESC`, [id]);
        ...
        // Admin
        const result = await query(`SELECT m.*, uc.name as caregiver_name, uf.name as family_name, cr.care_type
       FROM matches m
       JOIN users uc ON uc.id = m.caregiver_id
       JOIN users uf ON uf.id = m.family_id
       LEFT JOIN care_requests cr ON cr.id = m.request_id
...
router.put('/:id/accept', requireAuth, async (req, res) => {
    try {
        const result = await query(`UPDATE matches SET status = 'accepted' WHERE id = $1 AND caregiver_id = $2 RETURNING *`, [req.params.id, req.user.id]);
...
router.put('/:id/decline', requireAuth, async (req, res) => {
    try {
        const result = await query(`UPDATE matches SET status = 'declined' WHERE id = $1 AND caregiver_id = $2 RETURNING *`, [req.params.id, req.user.id]);
        if (result.rows.length === 0)
            return res.status(404).json({ error: 'Match not found' });
        res.json({ match: result.rows[0] });
    }
...
router.post('/:id/unlock-messaging', requireAuth, async (req, res) => {
    try {
        ...
        const result = await query(`UPDATE matches SET messaging_unlocked = true, care_date = NULL WHERE id = $1 AND family_id = $2 RETURNING *`, [req.params.id, req.user.id]);
        if (result.rows.length === 0)
            return res.status(404).json({ error: 'Match not found' });
