...
router.get('/:id/messages', requireAuth, async (req, res) => {
    try {
        ...
        const messagesResult = await query(`SELECT m.id, m.conversation_id, m.sender_id, m.content, m.created_at,
              u.name as sender_name, u.photo_url as sender_photo
       FROM messages m
       JOIN users u ON u.id = m.sender_id
       WHERE m.conversation_id = $1
       ORDER BY m.created_at ASC`, [req.params.id]);
        const msgs = messagesResult.rows.map((m) => ({
            id: m.id,
