...
router.put('/notifications', requireAuth, async (req, res) => {
    try {
        ...
        const prevPrefs = prevRes.rows[0]?.notification_prefs || {};
...
router.put('/profile', requireAuth, async (req, res) => {
    try {
        ...
        const result = await query(`UPDATE users SET ${updates.join(', ')} WHERE id = $${idx} RETURNING id, name, email, role, photo_url, phone, address`, params);
        const u = result.rows[0];
