...
router.get('/', requireAuth, async (req, res) => {
    try {
        ...
        if (req.user.role === 'family') {
            result = await query(`SELECT cr.id, cr.care_type, cr.details, cr.location, cr.zip, cr.status, cr.created_at,
...
router.get('/:id', requireAuth, async (req, res) => {
    try {
        const result = await query(`SELECT cr.*, u.name as family_name,
              (SELECT COUNT(*) FROM matches WHERE request_id=cr.id) as match_count
       FROM care_requests cr
       JOIN users u ON u.id = cr.family_id
       WHERE cr.id = $1 AND (cr.family_id = $2 OR $3 = 'admin')`, [req.params.id, req.user.id, req.user.role]);
