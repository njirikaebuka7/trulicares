...
router.get('/stats', requireAdmin, async (_req, res) => {
    try {
      ...
      `),
        ...
        const monthlyGrowth = growthResult.rows.map((r) => ({
            month: r.month,
...
router.get('/users', requireAdmin, async (req, res) => {
    try {
        ...
        const result = await query(`SELECT u.id, u.name, u.email, u.role, u.status, u.photo_url, u.created_at,
...
router.put('/users/:id', requireAdmin, async (req, res) => {
    try {
        ...
        const result = await query(`UPDATE users SET ${updates.join(', ')} WHERE id = $${idx} RETURNING id, name, email, role, status`, params);
...
router.put('/users/:id/suspend', requireAdmin, async (req, res) => {
    try {
        const result = await query(`UPDATE users SET status = 'suspended', updated_at = NOW() WHERE id = $1 RETURNING id, name, status`, [req.params.id]);
        if (result.rows.length === 0)
            return res.status(404).json({ error: 'User not found' });
        res.json({ user: result.rows[0] });
    }
...
router.put('/users/:id/restore', requireAdmin, async (req, res) => {
    try {
        const result = await query(`UPDATE users SET status = 'active', updated_at = NOW() WHERE id = $1 RETURNING id, name, status`, [req.params.id]);
        if (result.rows.length === 0)
            return res.status(404).json({ error: 'User not found' });
        res.json({ user: result.rows[0] });
    }
...
router.get('/verification-queue', requireAdmin, async (_req, res) => {
    try {
        const result = await query(`SELECT vq.id, vq.caregiver_id, vq.specialty, vq.experience, vq.documents,
...
router.put('/verification/:id', requireAdmin, async (req, res) => {
    try {
        const { status } = req.body;
        if (!['approved', 'rejected'].includes(status)) {
            return res.status(400).json({ error: 'Status must be "approved" or "rejected"' });
        }
        const result = await query(`UPDATE verification_queue SET status = $1 WHERE id = $2 RETURNING id, caregiver_id, status`, [status, req.params.id]);
        if (result.rows.length === 0)
...
router.get('/reports', requireAdmin, async (_req, res) => {
    try {
        const result = await query(`SELECT r.id, r.type, r.description, r.evidence, r.status, r.priority, r.created_at,
...
router.put('/reports/:id', requireAdmin, async (req, res) => {
    try {
        ...
        const result = await query(`UPDATE reports SET status = $1 WHERE id = $2 RETURNING id, status`, [status, req.params.id]);
        if (result.rows.length === 0)
