...
router.get('/', requireAuth, async (req, res) => {
    try {
        ...
        let result;
        if (role === 'caregiver') {
            result = await query(`SELECT s.id, s.family_name, s.service, s.date_label, s.time_label, s.location, s.status,
         ...
         ORDER BY s.created_at DESC`, [id]);
            ...
            result = await query(`SELECT s.id, s.service, s.date_label, s.time_label, s.location, s.status,
