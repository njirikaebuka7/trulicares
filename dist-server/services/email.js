...
function getResend() {
    if (!resend) {
        ...
        if (!apiKey) {
            ...
            return { emails: { send: async (opts) => { console.log('[EMAIL MOCK]', opts); return { data: { id: 'mock' }, error: null }; } } };
        }
...
export async function sendMatchNotification(to, name, caregiverName) {
    return getResend().emails.send({
        from: FROM,
        to,
...
export async function sendJobRequestNotification(to, name, familyName, service) {
    return getResend().emails.send({
        from: FROM,
        to,
...
export async function sendMessageNotification(to, name, senderName) {
    return getResend().emails.send({
        from: FROM,
        to,
    ...
    return getResend().emails.send({
      ...
      <div style="font-family:sans-serif;max-width:600px;margin:0 auto">
        ...
        <p>Hi ${name}, ${approved
            ...
            : 'your verification requires additional information. Please check your dashboard for details.'}</p>
