...
router.get('/', requireCaregiver, async (req: AuthRequest, res) => {
  try {
    ...
    const result = await query(
      ...
      [id, startOfWeek.toISOString(), startOfLastWeek.toISOString(), startOfMonth.toISOString(), startOfLastMonth.toISOString(), startOfYear.toISOString()]
    );
    ...
    const weeklyMap = new Map(weeklyResult.rows.map((r: any) => [r.day, parseInt(r.total) / 100]));
